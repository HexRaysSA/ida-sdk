...
///@}
  ...
  /// for example, byte ptr [epb+32_bit_offset]  will have the #dt_byte type.
  op_dtype_t dtype = 0;
...
#define CTXF_HIDDEN_ADDR  0x100000 ///< generate an hidden addr tag at the beginning of the line
#define CTXF_BIT_PREFIX   0x200000 ///< generate a line prefix with a bit offset, e.g.: 12345678.3
#define CTXF_UNHIDE       0x400000 ///< display hidden objects (segment, function, range)
...
#define OOF_ADDR        0x0080      ///< output x.addr, otherwise x.value
                                    ///< OOF_WIDTHMASK must be explicitly specified with it
...
#define GH_PRINT_ALL_BUT_BYTESEX (GH_PRINT_PROC_AND_ASM | GH_PRINT_HEADER)
  ...
  /// This function is called to generate the opening lines of a function,
  /// including border, comments, attributes, and the function name.
  virtual void idaapi gen_function_header(ea_t func_ea) newapi;

  /// Generate function footer lines.
  /// This function is called to generate the closing lines of a function,
...
/// \return success
/// \retval true    ok
/// \retval false   can't represent as floating point number
