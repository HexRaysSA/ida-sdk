...
public:
  ...
  /// Return 'true' if the value is unknown because the register has too
  /// many values.
...
/// (CREF!=fl_U).
/// Try to update the register tracker cache after this change.
...
/// Update the regtracker xrefs cache after this change.
/// If TO == BADADDR then clear the entire xrefs cache.
