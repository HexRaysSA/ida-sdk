...
typedef qvector<callcnv_t> callcnvs_t;
...
struct compiler_info_t
{
  ...
  inline callcnv_t get_cc() const;
  inline void set_cc(callcnv_t cc);
};
...
inline callcnv_t inf_get_callcnv() { return callcnv_t(getinf(INF_CALLCNV)); }
inline bool inf_set_callcnv(callcnv_t _v) { return setinf(INF_CALLCNV, ssize_t(_v)); }
...
inline bool inf_get_cc(compiler_info_t *out) { return getinf_buf(INF_COMPILER_INFO, out, sizeof(*out)) > 0; }
inline bool inf_set_cc(const compiler_info_t &_v) { return setinf_buf(INF_COMPILER_INFO, &_v, sizeof(_v)); }
...
//--------------------------------------------------------------------------
/// Types of events that be hooked to with \ref hook_event_listener()
enum hook_type_t
{
...
{
  const char *name;             ///< human-readable name. if null, then
                                ///< the field will not be merged as part of INF.
