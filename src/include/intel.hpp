...
#define PRF_MOVE   0x0001 // Entry describes a register save by a move instruction
...
inline bool is_msabi(callcnv_t cc)
{
  return cc == CM_CC_STDCALL || is_msabi();
}

inline int pc_shadow_area_size(callcnv_t cc)
{
...
#define AFIDP_NO_USE_ANY_IND_OP 0x10000 // resolve indirect operands of 'jmpni/callni'
                                        // only for read-only memory
                                        // 'NO' is used to simplify upgrading existing idbs
...
inline bool should_af_use_any_ind_op(void) { return (pc_module_t::get_idpflags() & AFIDP_NO_USE_ANY_IND_OP) == 0; }
