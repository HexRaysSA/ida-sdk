...
/*! \file typeinf.hpp
  ...
  \brief Type information in IDA.

  In IDA, types are represented by and manipulated through ::tinfo_t objects.

  A ::tinfo_t can represent a simple type (e.g., `int`, `float`),
  a complex type (a structure, enum, union, typedef), or even
  an array, or a function prototype.
  ...
  All throughout this file, there are certain terms that will keep
  appearing:
  ...
  The ::tinfo_t type provides a lot of useful methods already,
  but it's possible to achieve even more by retrieving its
  contents into the container classes:
  ...
  ::tinfo_t objects can be attached to a ::til_t library, or can be
  created without using any til_t.
  ...
  This code manipulates a "detached" ::tinfo_t object, which does not depend on
  any ::til_t file. However, any complex type will require a ::til_t file.
  In IDA, there is always a default til_t file for each idb file.
  This til_t file can be specified by nullptr.

  On the other hand, the following code manipulates an "attached"
  ::tinfo_t object, and any operation that modifies it, will
  also modify it in the hosting ::til_t:
  ...
  You can check if a ::tinfo_t instance is attached to a type in a til_t file
...
const callcnv_t  CM_CC_INVALID  = 0x00;  ///< this value is invalid
const callcnv_t  CM_CC_UNKNOWN  = 0x10;  ///< unknown calling convention
const callcnv_t  CM_CC_VOIDARG  = 0x20;  ///< function without arguments
                                         ///< if has other cc and argnum == 0,
                                         ///< represent as f() - unknown list
const callcnv_t  CM_CC_CDECL    = 0x30;  ///< stack
const callcnv_t  CM_CC_ELLIPSIS = 0x40;  ///< cdecl + ellipsis
const callcnv_t  CM_CC_STDCALL  = 0x50;  ///< stack, purged
const callcnv_t  CM_CC_PASCAL   = 0x60;  ///< stack, purged, reverse order of args
const callcnv_t  CM_CC_FASTCALL = 0x70;  ///< stack, purged (x86), first args are in regs (compiler-dependent)
const callcnv_t  CM_CC_THISCALL = 0x80;  ///< stack, purged (x86), first arg is in reg (compiler-dependent)
const callcnv_t  CM_CC_SWIFT    = 0x90;  ///< (Swift) arguments and return values in registers (compiler-dependent)
const callcnv_t  CM_CC_SPOILED  = 0xA0;  ///< This is NOT a cc! Mark of __spoil record
...
const callcnv_t  CM_CC_GOLANG   = 0xB0;  ///< (Go) arguments and return value reg/stack depending on version
const callcnv_t  CM_CC_RESERVE3 = 0xC0;  ///< reserved; used for internal needs
const callcnv_t  CM_CC_SPECIALE = 0xD0;  ///< ::CM_CC_SPECIAL with ellipsis
const callcnv_t  CM_CC_SPECIALP = 0xE0;  ///< Equal to ::CM_CC_SPECIAL, but with purged stack
const callcnv_t  CM_CC_SPECIAL  = 0xF0;  ///< usercall: locations of all arguments
...
struct custom_callcnv_t
{
  ...
  uint64 flags = 0;
#define CCI_VARARG 0x0001  ///< is variadic?
#define CCI_PURGE  0x0002  ///< purges arguments?
#define CCI_USER   0x0004  ///< is usercall? not tested
  qstring name;            ///< the name is used as a keyword in the function prototype
  uint32 abibits = 0;      ///< abibits to be used for the calling convention
  ...
  /// This function is used during parsing or deserializing a function prototype
  /// to verify semantic limitations of the prototype (for example, returning
  /// arrays is forbidden in C)
  /// \param[in] fti function prototype
  /// \param[out] reterr buffer for error message
  ...
  /// This function must fill all fti->at(i).argloc instances.
  /// It may be called for variadic functions too, in calc_varglocs fails.
  /// \param fti function prototype
  /// \return success
  virtual bool calc_arglocs(func_type_data_t *fti) const = 0;

  /// Discover variadic arguments.
  /// This function is called only for variadic functions.
  /// It is currently used by the decompiler.
  /// \param fti function prototype. find_varargs() should append the discovered
  ///            variadic arguments to it.
  /// \param call_ea address of the call instruction
  /// \param blk microcode block with the call instruction
  /// \return >0 - total number of arguments after the call
  ///         <0 - failure
  ///        ==0 - means to use the standard algorithm to discover variadic args
  ...
  /// This function must fill all fti->at(i).argloc instances and provide
  /// more detailed info about registers and stkargs.
  /// \param fti          function prototype
  /// \param[out] regs    buffer for hidden register arguments, may be nullptr
  /// \param[out] stkargs buffer for hidden stack arguments, may be nullptr
  ...
  /// Some compilers decorate names depending on the calling convention.
  /// This function provides the means to handle it for custom callcnvs.
  /// Please note that this is about name decoration (C), not name mangling (C++).
  ...
  /// \param fti     function prototype
  /// \return <0-failure, >=0-ok, 2-made substantial changes
...
///              this object must be alive until unregister_custom_callcnv(),
///              after that it can destroyed
/// \return CM_CC_INVALID means failure:
///          - bad ccinf.name
///          - ccinf.name already exists
///          - the calling convention is special (usercall, purging, vararg)
///            and there are too many of them already
...
/// \return CM_CC_INVALID is not found
...
/// \param codes output buffer for the convention codes
/// The two output buffers correspond to each other.
/// \return number of the calling conventions added to the output buffers
...
public:
  argreg_policy_t policy = ARGREGS_POLICY_UNDEFINED; ///< argument policy
  int nregs = 0;            ///< max number of registers that can be used in a call
...
#define HTI_SEMICOLON   0x00200000     ///< do not complain if the terminating semicolon is absent
#define HTI_STANDALONE  0x00400000     ///< should parse standalone declaration,
...
public:
  ...
  /// Get type alignment
  /// This function returns the effective type alignment. Zero means error.
...
///@}
  ...
  /// \param[out] out  udm_t storage. Must not be nullptr
  /// \param name      member name. Must not be nullptr
  /// \return the index of the found member, or -1
  ...
  /// \param[out] out enum type member, may be nullptr
  /// \param      name enum member name. Must not be nullptr
  ...
  ///
  /// \param name   Member name. Must not be nullptr or empty.
  /// \param type   Member type. Must not be empty. Can be any valid
  ///               udt member type, like a pointer, array, etc.
  /// \param offset Member offset in bits. It is the caller's
  ...
  /// Add a structure/union member.
  ///
  /// The member's type will be created from 'type', and
  /// its size will be computed automatically.
  ///
  /// \param name   Member name. Must not be nullptr or empty.
  ...
  /// Add a structure/union member.
  ///
  /// The member's type will be created by parsing 'type', and
  /// its size will be computed automatically.
  ///
  /// \param name   Member name. Must not be nullptr or empty.
  /// \param type   Member type. A valid C type declaration.
  /// \param offset Member offset in bits. It is the caller's
  ///               responsibility to specify correct offsets.
  /// \param etf_flags \ref ETF_ flags
  /// \param times how many times to add. if times > 1, the member name will be
  ...
  /// Expand/shrink a structure by adding/removing a gap
  /// before the specified member.
  ///
  /// For regular structures, either the gap can be accommodated by
  /// aligning the next member with an alignment directive, or an
  /// explicit "gap" member will be inserted.
  /// Also note that it is impossible to add a gap at the
  /// end of a regular structure.
  ///
  /// When it comes to fixed-layout structures, there is no need to
  /// either add new "gap" members or align existing members, since
  /// all members have a fixed offset.
  /// It is possible to add a gap at the end of a fixed-layout
  /// structure, by passing `-1` as index.
  ///
  /// \param idx index of the member
  ...
  /// Currently, tuples in IDA behave the same way as structures but they are
  /// returned in a different manner from functions. Also, 2 different tuples
  /// having the same members are considered to be equal.
  /// This function works only with structures (not unions).
  /// \param on
  ...
  /// \note
  /// 1. For non-bitmask enums, `bmask` is ignored
  /// 2. For bitmask enums:
  ...
  ///
  /// \param name the enumerator name (cannot be nullptr)
  /// \param etf_flags \ref etf_flag_t
  /// \return \ref tinfo_code_t
...
#define TCMP_DEEP_UDT 0x0100 ///< compare udt by member/attributes
///@}
...
///@}
  ...
  ///
  /// \param _name   Argument name. May be empty
  /// \param _type   Argument type. Must not be empty.
  /// \param _argloc Argument location. Can be empty.
...
struct edm_t // #edm
{
  ...
  ///
  /// \param _name  Enumerator name. Must not be empty.
  /// \param _value Enumerator value
  /// \param _cmt   Enumerator repeatable comment. May be empty.
...
struct udt_type_data_t : public udtmembervec_t // #udt
{
  ...
  /// \param      _type    Member type. Must not be empty.
  /// \param[in]  _offset  Member offset in bits. It is the caller's responsibility to specify correct offsets.
  ///
