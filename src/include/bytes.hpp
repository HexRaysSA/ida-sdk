...
/// This function returns the most commonly used bits of the flags.
/// However, it does not return the operand info for the operands beyond the
/// first two operands (0,1). If you need to deal with the operands (2..n),
/// then use get_flags().
/// It is customary to assign the return value to the variable named "F32",
/// to distinguish is from 64-bit flags.
/// \return 0 if address is not present in the program
inline flags64_t idaapi get_flags32(ea_t ea) { return get_flags_ex(ea, GFE_32BIT); }

/// Get flags value for address 'ea'.
/// The byte value is not included in the flags.
/// This function should be used if the operand types of any operand beyond
/// the first two operands is required.
/// This function is more expensive to use than get_flags32()
/// \return 0 if address is not present in the program
inline flags64_t idaapi get_flags(ea_t ea) { return get_flags_ex(ea, 0); }

/// Get full flags value for address 'ea'.
/// This function returns the byte value in the flags as well.
/// See FF_IVL and MS_VAL.
/// This function is more expensive to use than get_flags()
/// \return 0 if address is not present in the program
...
inline THREAD_SAFE constexpr bool idaapi has_value(flags64_t F)  { return (F & FF_IVL) != 0; }
...
inline THREAD_SAFE constexpr bool idaapi has_extra_cmts(flags64_t F)   { return (F & FF_LINE) != 0; }
inline THREAD_SAFE constexpr bool idaapi f_has_extra_cmts(flags64_t f, void *) { return has_extra_cmts(f); }

/// Does the current byte have an indented comment?

inline THREAD_SAFE constexpr bool idaapi has_cmt(flags64_t F)    { return (F & FF_COMM) != 0; }
inline THREAD_SAFE constexpr bool idaapi f_has_cmt(flags64_t f, void *) { return has_cmt(f); }

/// Does the current byte have cross-references to it?

inline THREAD_SAFE constexpr bool idaapi has_xref(flags64_t F)     { return (F & FF_REF)  != 0; }
inline THREAD_SAFE constexpr bool idaapi f_has_xref(flags64_t f, void *) { return has_xref(f); }       ///< \copydoc has_xref()


/// Does the current byte have non-trivial (non-dummy) name?

inline THREAD_SAFE constexpr bool idaapi has_name(flags64_t F)   { return (F & FF_NAME) != 0; }
inline THREAD_SAFE constexpr bool idaapi f_has_name(flags64_t f, void *) { return has_name(f); }       ///< \copydoc has_name()
...
inline THREAD_SAFE constexpr bool idaapi has_dummy_name(flags64_t F) { return (F & FF_ANYNAME) == FF_LABL; }
inline THREAD_SAFE constexpr bool idaapi f_has_dummy_name(flags64_t f, void *) { return has_dummy_name(f); } ///< \copydoc has_dummy_name()


/// Does the current byte have auto-generated (no special prefix) name?

inline THREAD_SAFE constexpr bool idaapi has_auto_name(flags64_t F) { return (F & FF_ANYNAME) == FF_ANYNAME; }


/// Does the current byte have any name?

inline THREAD_SAFE constexpr bool idaapi has_any_name(flags64_t F) { return (F & FF_ANYNAME) != 0; }


/// Does the current byte have user-specified name?

inline THREAD_SAFE constexpr bool idaapi has_user_name(flags64_t F) { return (F & FF_ANYNAME) == FF_NAME; }
inline THREAD_SAFE constexpr bool idaapi f_has_user_name(flags64_t F, void *) { return has_user_name(F); } ///< \copydoc has_user_name()
...
inline THREAD_SAFE constexpr bool idaapi is_defarg0(flags64_t F) { return !is_flag_for_operand(F, FF_N_VOID, 0); }


/// Is the second operand defined? Initially operand has no defined representation

inline THREAD_SAFE constexpr bool idaapi is_defarg1(flags64_t F) { return !is_flag_for_operand(F, FF_N_VOID, 1); }


/// Is the first operand offset? (example: push offset xxx)

inline THREAD_SAFE constexpr bool idaapi is_off0(flags64_t F)    { return is_flag_for_operand(F, FF_N_OFF, 0); }


/// Is the second operand offset? (example: mov ax, offset xxx)

inline THREAD_SAFE constexpr bool idaapi is_off1(flags64_t F)    { return is_flag_for_operand(F, FF_N_OFF, 1); }


/// Is the first operand character constant? (example: push 'a')

inline THREAD_SAFE constexpr bool idaapi is_char0(flags64_t F)   { return is_flag_for_operand(F, FF_N_CHAR, 0); }


/// Is the second operand character constant? (example: mov al, 'a')

inline THREAD_SAFE constexpr bool idaapi is_char1(flags64_t F)   { return is_flag_for_operand(F, FF_N_CHAR, 1); }


/// Is the first operand segment selector? (example: push seg seg001)

inline THREAD_SAFE constexpr bool idaapi is_seg0(flags64_t F)    { return is_flag_for_operand(F, FF_N_SEG, 0); }


/// Is the second operand segment selector? (example: mov dx, seg dseg)

inline THREAD_SAFE constexpr bool idaapi is_seg1(flags64_t F)    { return is_flag_for_operand(F, FF_N_SEG, 1); }


/// Is the first operand a symbolic constant (enum member)?

inline THREAD_SAFE constexpr bool idaapi is_enum0(flags64_t F)   { return is_flag_for_operand(F, FF_N_ENUM, 0); }


/// Is the second operand a symbolic constant (enum member)?

inline THREAD_SAFE constexpr bool idaapi is_enum1(flags64_t F)   { return is_flag_for_operand(F, FF_N_ENUM, 1); }


/// Is the first operand an offset within a struct?

inline THREAD_SAFE constexpr bool idaapi is_stroff0(flags64_t F) { return is_flag_for_operand(F, FF_N_STRO, 0); }


/// Is the second operand an offset within a struct?

inline THREAD_SAFE constexpr bool idaapi is_stroff1(flags64_t F) { return is_flag_for_operand(F, FF_N_STRO, 1); }


/// Is the first operand a stack variable?

inline THREAD_SAFE constexpr bool idaapi is_stkvar0(flags64_t F) { return is_flag_for_operand(F, FF_N_STK, 0);  }


/// Is the second operand a stack variable?

inline THREAD_SAFE constexpr bool idaapi is_stkvar1(flags64_t F) { return is_flag_for_operand(F, FF_N_STK, 1); }


/// Is the first operand a floating point number?

inline THREAD_SAFE constexpr bool idaapi is_float0(flags64_t F) { return is_flag_for_operand(F, FF_N_FLT, 0); }


/// Is the second operand a floating point number?

inline THREAD_SAFE constexpr bool idaapi is_float1(flags64_t F) { return is_flag_for_operand(F, FF_N_FLT, 1); }


/// Does the first operand use a custom data representation?

inline THREAD_SAFE constexpr bool idaapi is_custfmt0(flags64_t F) { return is_flag_for_operand(F, FF_N_CUST, 0); }


/// Does the second operand use a custom data representation?

inline THREAD_SAFE constexpr bool idaapi is_custfmt1(flags64_t F) { return is_flag_for_operand(F, FF_N_CUST, 1); }
...
inline THREAD_SAFE constexpr flags64_t get_optype_flags0(flags64_t F) { return F & (MS_N_TYPE << get_operand_type_shift(0)); }


/// Get flags for second operand

inline THREAD_SAFE constexpr flags64_t get_optype_flags1(flags64_t F) { return F & (MS_N_TYPE << get_operand_type_shift(1)); }
...
inline THREAD_SAFE constexpr bool idaapi is_byte(flags64_t F)      { return is_data(F) && (F & DT_TYPE) == FF_BYTE; }      ///< #FF_BYTE
inline THREAD_SAFE constexpr bool idaapi is_word(flags64_t F)      { return is_data(F) && (F & DT_TYPE) == FF_WORD; }      ///< #FF_WORD
inline THREAD_SAFE constexpr bool idaapi is_dword(flags64_t F)     { return is_data(F) && (F & DT_TYPE) == FF_DWORD; }     ///< #FF_DWORD
inline THREAD_SAFE constexpr bool idaapi is_qword(flags64_t F)     { return is_data(F) && (F & DT_TYPE) == FF_QWORD; }     ///< #FF_QWORD
inline THREAD_SAFE constexpr bool idaapi is_oword(flags64_t F)     { return is_data(F) && (F & DT_TYPE) == FF_OWORD; }     ///< #FF_OWORD
inline THREAD_SAFE constexpr bool idaapi is_yword(flags64_t F)     { return is_data(F) && (F & DT_TYPE) == FF_YWORD; }     ///< #FF_YWORD
inline THREAD_SAFE constexpr bool idaapi is_zword(flags64_t F)     { return is_data(F) && (F & DT_TYPE) == FF_ZWORD; }     ///< #FF_ZWORD
inline THREAD_SAFE constexpr bool idaapi is_tbyte(flags64_t F)     { return is_data(F) && (F & DT_TYPE) == FF_TBYTE; }     ///< #FF_TBYTE
inline THREAD_SAFE constexpr bool idaapi is_float(flags64_t F)     { return is_data(F) && (F & DT_TYPE) == FF_FLOAT; }     ///< #FF_FLOAT
inline THREAD_SAFE constexpr bool idaapi is_double(flags64_t F)    { return is_data(F) && (F & DT_TYPE) == FF_DOUBLE; }    ///< #FF_DOUBLE
inline THREAD_SAFE constexpr bool idaapi is_pack_real(flags64_t F) { return is_data(F) && (F & DT_TYPE) == FF_PACKREAL; }  ///< #FF_PACKREAL
inline THREAD_SAFE constexpr bool idaapi is_strlit(flags64_t F)    { return is_data(F) && (F & DT_TYPE) == FF_STRLIT; }    ///< #FF_STRLIT
...
inline THREAD_SAFE constexpr bool idaapi f_is_dword(flags64_t F, void *)     { return is_dword(F); }                       ///< See is_dword()
inline THREAD_SAFE constexpr bool idaapi f_is_qword(flags64_t F, void *)     { return is_qword(F); }                       ///< See is_qword()
inline THREAD_SAFE constexpr bool idaapi f_is_oword(flags64_t F, void *)     { return is_oword(F); }                       ///< See is_oword()
inline THREAD_SAFE constexpr bool idaapi f_is_yword(flags64_t F, void *)     { return is_yword(F); }                       ///< See is_yword()
inline THREAD_SAFE constexpr bool idaapi f_is_tbyte(flags64_t F, void *)     { return is_tbyte(F); }                       ///< See is_tbyte()
inline THREAD_SAFE constexpr bool idaapi f_is_float(flags64_t F, void *)     { return is_float(F); }                       ///< See is_float()
inline THREAD_SAFE constexpr bool idaapi f_is_double(flags64_t F, void *)    { return is_double(F); }                      ///< See is_double()
inline THREAD_SAFE constexpr bool idaapi f_is_pack_real(flags64_t F, void *) { return is_pack_real(F); }                   ///< See is_pack_real()
inline THREAD_SAFE constexpr bool idaapi f_is_strlit(flags64_t F, void *)    { return is_strlit(F); }                      ///< See is_strlit()
inline THREAD_SAFE constexpr bool idaapi f_is_struct(flags64_t F, void *)    { return is_struct(F); }                      ///< See is_struct()
inline THREAD_SAFE constexpr bool idaapi f_is_align(flags64_t F, void *)     { return is_align(F); }                       ///< See is_align()
inline THREAD_SAFE constexpr bool idaapi f_is_custom(flags64_t F, void *)    { return is_custom(F); }                      ///< See is_custom()


/// Do the given flags specify the same data type?

inline THREAD_SAFE constexpr bool idaapi is_same_data_type(flags64_t F1, flags64_t F2) { return ((F1 ^ F2) & DT_TYPE) == 0; }
