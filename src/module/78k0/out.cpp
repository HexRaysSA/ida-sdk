...
void nec78k0_t::N78K_segstart(outctx_t &ctx, ea_t seg_ea) const
{
  segment_info_t si;
  get_segment_info(&si, seg_ea);
  const char *SegType = si.get_type() == SEG_CODE ? "CSEG"
                      : si.get_type() == SEG_DATA ? "DSEG"
                      : "RSEG";
  // line: RSEG <NAME>
  qstring sn;
  get_segment_name(&sn, seg_ea, 1);
