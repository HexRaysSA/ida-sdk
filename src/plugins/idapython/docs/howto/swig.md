...
Typically (the generated wrappers live under your build directory):
`cp -R <build>/.../wrappers/ /tmp/wrappers-before`
<recompile...>
`git diff /tmp/wrappers-before/ <build>/.../wrappers/`
...
2. add `zzz` to the `MODULE_NAMES` list in `cmake/idapython_body.cmake`
...
4. [build](../../BUILDING.md) — drift in `api_contents.brief` is auto-applied; just `git add` and commit it after the build.
